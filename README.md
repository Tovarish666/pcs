...
По шагам: проверяет, что на ВМ стоит софт mobileproxy.space → спрашивает ссылку на таблицу (если её ещё нет) и запоминает → собирает шаблон ВМ модема `9999`, если его нет → создаёт по таблице все модемы и втыкает их в эту ВМ. Готовые модемы не трогает, так что команду можно повторять.

Без софта mp.space команда откажется работать: модемы бы создались и воткнулись, но поднимать интерфейсы и раздавать `192.168.<N>.100` некому — это делают их собственные скрипты. Сначала `pcs mp-install`.
...
Плюс 219 локальных проверок на разбор таблицы, конфиги, режим показа и заглушки.
