...
## Весь путь, по порядку

```bash
pcs
```

Меню идёт сверху вниз ровно так, как разворачивается сервер:

1. **Создать ВМ Ubuntu 24.04** — `pcs vm-create`. Номер предлагается свободной тысячей: 1000, 2000, 3000…
2. **Установить софт mobileproxy.space** — `pcs mp-install`.
3. **Развернуть виртуальные модемы** — `pcs modem-deploy`: спросит ссылку на Google-таблицу, соберёт шаблон ВМ модема (9999) и создаст по таблице все модемы, воткнув их в эту ВМ.
4. **Проверить** — `pcs modem-check`.

## Кто чей: номера ВМ

ВМ mobileproxy.space нумеруются тысячами, а её модемы живут в этой же тысяче:

| ВМ mobileproxy.space | модем `n=1` | модем `n=2` | … |
|---|---|---|---|
| 1000 | 1001 | 1002 | … |
| 3000 | 3001 | 3002 | … |
| 10000 | 10001 | 10002 | … |

По номеру ВМ сразу видно, чей это модем. Состояние тоже разложено по серверам: `/etc/pcs/modem/<ВМ>/` — там и ссылка на таблицу, и файл на каждый модем. Таблица у каждой ВМ своя, шаблон модема — один на хост.
...
- Номер по умолчанию — свободная тысяча (1000, 2000, 3000…): её займут модемы этой ВМ.
...
```bash
pcs mp-install --proxy http://user:pass@1.2.3.4:3128
```

Семь шагов: утилиты PCS на ВМ → **проверка доступа к зеркалам mp.space** → выключение авто-обновлений apt и модули ядра для USB-модемов → `install.sh` от mp.space → `auth.mp` → `setup-modem-management.sh` от mp.space → перезагрузка, DNS и проверка служб. В конце печатается сводка для ЛК.

- **Про прокси.** Сайты mp.space у части провайдеров не открываются, и тогда их скрипты просто не качаются. PCS сначала пробует с самой ВМ все зеркала напрямую и, только если не ответило ни одно, спрашивает HTTP-прокси — адрес проверяется тут же и запоминается в состоянии ВМ (права 600). Прокси нужен лишь на время скачивания: через него идут `wget` скриптов mp.space и `apt`. Задать сразу — `--proxy`, обойтись без вопроса — `--no-proxy`.
...
## pcs modem-deploy — развернуть модемы по таблице
...
```bash
pcs modem-deploy --source 'https://docs.google.com/spreadsheets/d/ВАШ_ID/edit#gid=0'
```

```bash
pcs modem-deploy --source 'https://docs.google.com/spreadsheets/d/ДРУГОЙ_ID/edit#gid=0' --recreate
```

```bash
pcs modem-deploy --vm 3000
```

По шагам: спрашивает ссылку на таблицу (если её ещё нет) и запоминает → поднимает потолок USB на ВМ под размер таблицы → собирает шаблон ВМ модема `9999`, если его нет → создаёт по таблице все модемы и втыкает их в эту ВМ. Готовые модемы не трогает, так что команду можно повторять.

ВМ не обязана быть сервером mobileproxy.space — модемы воткнутся в любую. Просто адреса `192.168.<N>.100` на той стороне выдаёт софт mp.space; если его нет, команда об этом скажет, а поднимать интерфейсы вам.

- `--source <ссылка>` — сменить таблицу этой ВМ. Таблица у каждой ВМ mobileproxy.space своя.
- `--recreate` — снести все модемы этой ВМ и создать заново по текущей таблице: так меняют таблицу целиком. Спрашивает подтверждение и перечисляет, что снесёт.
- `--template 9998` — другой номер шаблона, если `9999` занят.
- `--no-attach` — создать ВМ модемов, но в сервер не втыкать.
...
Ссылка своя у каждой ВМ mobileproxy.space (`--vm ID` — для не активной). Ссылку можно давать прямо из адресной строки браузера — PCS сам приведёт её к CSV. Таблица должна быть доступна по ссылке (или опубликована), иначе Google отдаёт HTML и PCS об этом скажет. Вместо ссылки принимается локальный файл — удобно для проверки.
...
pcs modem-template --id 9999 --storage local-lvm --bridge vmbr0 \
                   --ram 384 --cores 1 --disk 4 --singbox 1.10.0
```
...
- Шаблон один на хост: модемы всех ВМ mobileproxy.space — его связанные клоны.
- Номера ВМ модемов от шаблона не зависят: их задаёт ВМ-сервер (ВМ 3000 → модемы 3001, 3002…).
- `--replace` пересобирает шаблон с нуля. Связанные клоны на нём держатся, так что сначала снеси модемы (`pcs modem-deploy --recreate` сделает это сам).
...
- Номер ВМ модема — номер ВМ mobileproxy.space плюс `n` (ВМ 3000, `n=201` → ВМ 3201). Другой: `--id`.
- `--vm ID` — чей это модем, если активная ВМ не та.
...
## pcs vm-usb-ports — сколько модемов принимает ВМ
...
```bash
pcs vm-usb-ports --modems 80
```

```bash
pcs vm-usb-ports --revert
```

Ядро Ubuntu принимает **ровно 8** устройств USB/IP: `CONFIG_USBIP_VHCI_HC_PORTS=8` и `CONFIG_USBIP_VHCI_NR_HCS=1` — константы времени сборки, параметра модуля для них нет. Девятый модем не втыкается вовсе, а в `dmesg` появляется `port N already used`.

Команда пересобирает `vhci-hcd` из исходников того же ядра с числами, с которыми ядро собирает Debian: **15 портов × 8 контроллеров = 120 модемов**. Модули кладутся в `/lib/modules/<ядро>/updates` и отдаются `dkms`, поэтому переживают перезагрузку и смену ядра.

- `--modems N` — ничего не делает, если портов и так хватает; иначе пересобирает.
- `--ports` / `--controllers` — свои числа. Осторожно: ядерный Kconfig ограничивает порты числом 31, а на 32 контроллерах ядро падает в `status_show` (проверено), поэтому команда их не принимает.
- Пересборка на пару секунд снимает воткнутые модемы, службы `vmodem-attach@` возвращают их сами.
- `pcs modem-deploy` вызывает это сам, по числу строк в таблице.
...
- Состояние: `/etc/pcs/mp/<vmid>.conf` (права 600), активная ВМ — `/etc/pcs/mp/active`; шаблон модема — `/etc/pcs/modem/template.conf`, а всё про модемы конкретной ВМ — в `/etc/pcs/modem/<vmid>/` (ссылка на таблицу и по файлу на модем).
...
Плюс 226 локальных проверок на разбор таблицы, конфиги, режим показа и заглушки.
