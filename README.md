...
## pcs modem-update — свежие утилиты на ВМ модемов
...
Кладёт на ВМ модемов свежие `vmodem`, `vmodem-api`, `vmodem-setup` из этой версии PCS: модемы, созданные раньше, остаются со старыми, а новые получают их при создании. `--restart` ещё и поднимает модем заново (`vmodem up`) — связь с сервером прервётся на несколько секунд, зато починки применятся сразу.
...
- `pcs modem-check --rotate` меняет IP той же последовательностью, что и mp.space для типа 3: `94.25.229.97 → 94.25.228.208` за пять секунд.
...
- `pcs modem-check` на всех шести: резолвер, USB, адрес, веб-морда и свой внешний адрес у каждого.
...
Плюс 196 локальных проверок на разбор таблицы, конфиги, режим показа и заглушки.
