...
**Что уже проверено, а что нет.** `vmodem-api` проверен по-настоящему: на заглушках модема и SOCKS5 гоняются 20 проверок. Ещё 135 проверок — на разбор таблицы, конфиги и режим показа `vmodem`, cloud-init шаблона и клона. Всё, что происходит в ядре (гаджет, USB/IP, sing-box), а также создание ВМ на живом Proxmox ещё не гонялись.
