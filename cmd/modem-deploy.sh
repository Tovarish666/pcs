...
#
#  Один пункт на весь путь, после vm-create и mp-install:
#    1. ссылка на Google-таблицу с модемами (n, real, proxy) — своя у каждой
#       ВМ mobileproxy.space, запоминается
#    2. шаблон ВМ модема (по умолчанию 9999) — собирается, если его ещё нет
#    3. модемы по таблице — создаются, поднимаются и втыкаются в эту ВМ
#
