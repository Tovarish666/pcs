...
#
#  Номер по умолчанию — свободная тысяча: 1000, 2000, 3000… Модемы этой ВМ
#  займут её тысячу (ВМ 3000 → модемы 3001, 3002, 3003…), поэтому номера и
#  раздаются с таким запасом.
# ═══════════════════════════════════════════════════════════════════════════
set -uo pipefail
...
ask O_ID "VM ID (её тысяча уйдёт под модемы)" "$(pve_next_server_id)"
[[ "$O_ID" =~ ^[0-9]+$ ]] || die "VM ID должен быть числом"
(( O_ID % 1000 == 0 )) || warn "номер не кратен 1000: модемы получат ${O_ID}+n — следи, чтобы не пересеклись с чужими ВМ"
